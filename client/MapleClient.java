...
public class MapleClient {
	...
	public void updateHWID(String newHwid) {
        ...
        public void setDaily(){
			...
			Connection con = DatabaseConnection.getConnection();
			try (PreparedStatement ps = con.prepareStatement("UPDATE accounts SET lastdaily = ? WHERE id = ?")) {
				ps.setInt(1, day);
        ...
        public boolean canDaily(){
            ...
            try {
			Connection con = DatabaseConnection.getConnection();
			PreparedStatement ps = con.prepareStatement("SELECT lastdaily FROM accounts WHERE id = ?");
				ps.setInt(1, getAccID());                              
				ResultSet rs = ps.executeQuery();
                                if (!rs.next()) {
				rs.close();
				...
				throw new RuntimeException("getLoginState - MapleClient");
                                ...
                                int rewardday = rs.getInt("lastdaily");
                             //   getPlayer().dropMessage("Day : " + rewardday);                            
                       if(rewardday != currentday){
                          // getPlayer().dropMessage(6,"Can Dailyreward");
                           setDaily();
                           return true;
                       }
                       else{
                         //  getPlayer().dropMessage(6,"Cant Dailyreward"); 
                           return false;
                       }
